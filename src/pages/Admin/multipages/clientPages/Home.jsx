...
import { faCircleUser, faStar, faUser, faUserTie } from "@fortawesome/free-solid-svg-icons";
...
function Home() {
    const data = [
          ...
          <div className="w-full flex flex-col md:flex-row justify-between items-center bg-primary-200 p-4 shadow-md rounded-lg mt-4 border border-green-500">
            <div className="text-center md:text-right">
              <div className="flex items-center justify-center md:justify-start gap-2">
                <FontAwesomeIcon icon={faUserTie} size="lg" className="text-black" />
                <h1 className="font-bold text-xl sm:text-2xl">اهلا: <span className="text-green-500">عمرو مندور</span></h1>
              </div>
              <p className="mt-2  text-lg bg-yellow-200 px-4 py-1 rounded-md shadow-md inline-block">
                لديك ٢ اشعار جديد
              </p>
            </div>
            <p className="mt-4 md:mt-0 text-xl text-gray-700"> متدرب</p>
          </div>
      ...
      <div className="flex flex-col items-center justify-center gap-4 w-full p-6 bg-primary-200 border border-green-500">
        <h1 className="text-2xl font-bold text-gray-700">تقييم المدرب</h1>
        ...
        <div className="flex justify-between w-full items-center mt-4 px-4">
          ...
          <span className="text-lg font-semibold"> <span className="text-yellow-600">4.5</span>/5 </span>
          <FontAwesomeIcon icon={faCircleUser} size="2x" className="text-black" />    
        </div></div>
      </div>
    
      {/* قسم تقييم الأداء */}
      <div className=" ">
        <h2 className="text-center text-2xl font-bold text-gray-700 mb-4 mt-7">تقييم الأداء</h2>
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={index} className="hover:bg-gray-100 text-center">
                  ...
                  <td className="border border-gray-400 p-3">{item.teachers}</td>
                </tr>
              ))}
            </tbody>
          </table>
