...
import styles from './TrainingAndEducation.module.css'; // تأكد من مسار الملف الصحيح
import logoSS from '../../assets/imgs/logo-SS - Copy.png'; // تأكد من مسار الشعار صحيح
...
const TrainingPlanCard = ({ plan }) => (
    <div className=''>
        <div className={`${styles.planCard} `}>
            ...
            <Link to={ ROUTES.RESERVATION} className={styles.button}>احجز الآن</Link>
        </div>
    </div>
);
                ...
                <div className="w-full lg:w-2/3 flex flex-col items-start space-y-6 lg:pr-8">
                    ...
                    <ul className={`${styles.featuresList} space-y-2`}>
                        <li>
                            ...
                            <span style={{ color: `var(--primary-color-light)`, fontWeight: 'bold', padding: '0 2px' }}>مدرب شخصي</span>
                        </li>
                        <li>
                            <FontAwesomeIcon icon={faCheck} className={styles.icon} />
                    ...
                    <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mt-6">
                        <button className={`${styles.buttonPersonal} font-semibold px-6 py-3 mx-2 rounded-lg shadow-lg`}>
                            احجز محاضرة تجريبية
                        </button>
                        <button className={`${styles.buttonTrial} font-semibold px-6 py-3 mx-2 rounded-lg shadow-lg`}>
                            احجز مدرب شخصي
                        </button>
                    </div>
                </div>

                <div className={`${styles.card} w-full lg:w-1/3 flex justify-center items-center mt-8 lg:mt-0 card`}>
                    <img src={logoSS} alt="Logo" className={`${styles.rotatingLogo}`} />
                </div>
            </div>


            {/* بطاقة الخطط التدريبية */}
